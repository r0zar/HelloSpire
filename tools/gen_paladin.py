...
def JUDGE(n):  # Tyr Faith at 3+ deepens every Judged application by one
    return f"await PowerCmd.Apply<JudgedPower>(choiceContext, cardPlay.Target!, {n}m + (FaithTracks.Has(Owner, Deity.Tyr, 3) ? 1m : 0m), Owner.Creature, this);"
WARD = ("await CreatureCmd.GainBlock(Owner.Creature, DynamicVars[\"Warded\"].BaseValue, ValueProp.Move, cardPlay);\n"
        "await PowerCmd.Apply<WardedPower>(choiceContext, Owner.Creature, DynamicVars[\"Warded\"].BaseValue, Owner.Creature, this);")
BLESS = "await PulseAuras(choiceContext);"
...
G="[gold]"; g="[/gold]"
def card(name, title, cost, type_, rarity, target, vars_, body, up, desc, mp=False, req=None, kw=(), tips=(), gb=False, bless=False, stips=()):
    body = list(body); tips = list(tips); stips = list(stips)
    if bless:
        body.append(BLESS); desc = desc + "\n[gold]Blessed[/gold]."; stips.append("Blessed")
    return dict(name=name, title=title, cost=cost, type=type_, rarity=rarity, target=target, vars=vars_, body=body, up=up, desc=desc, mp=mp, req=req, kw=list(kw), tips=tips, gb=gb, stips=stips)
...
card("Brace","Brace",1,"Skill","Common","Self",[V("var","Warded",6)],[WARD],[UP("Warded",3)],f"Gain {{Warded:diff()}} {G}Warded{g}.",gb=True,tips=["WardedPower"]),
...
card("ShieldWall","Shield Wall",2,"Skill","Common","Self",[V("var","Warded",11)],[WARD],[UP("Warded",4)],f"Gain {{Warded:diff()}} {G}Warded{g}.",gb=True,tips=["WardedPower"]),
card("AuraOfProtection","Aura of Protection",1,"Power","Common","Self",[V("power","AuraOfProtectionPower",2)],[POWERUP,APPLY("AuraOfProtectionPower")],[UP("AuraOfProtectionPower",1)],f"At the start of your turn, all allies gain {{AuraOfProtectionPower:diff()}} {G}Block{g}.",tips=["AuraOfProtectionPower"],stips=["Aura"]),
# ---------------- Common: Ilmater
card("Soothe","Soothe",1,"Skill","Common","AnyPlayer",[V("heal",6)],[HEAL_TARGET],[UP("Heal",3)],"Heal a player {Heal:diff()} HP."),
card("FlashOfLight","Flash of Light",1,"Skill","Common","AnyPlayer",[V("heal",5)],[HEAL_TARGET],[UP("Heal",3)],"Heal a player {Heal:diff()} HP.",bless=True),
...
card("Salve","Salve",0,"Skill","Common","Self",[V("heal",3)],[HEAL],[UP("Heal",2)],"Heal {Heal:diff()} HP.",bless=True),
...
card("Comfort","Comfort",1,"Skill","Common","AnyPlayer",[V("heal",4),V("cards",1)],[HEAL_TARGET,DRAW],[UP("Heal",2)],"Heal a player {Heal:diff()} HP.\nDraw {Cards:diff()} {Cards:plural:card|cards}.",bless=True),
# ---------------- Common: Tyr
card("Smite","Smite",1,"Attack","Common","AnyEnemy",[V("dmg",9)],[DMG()],[UP("Damage",3)],"Deal {Damage:diff()} damage."),
card("CrusaderStrike","Crusader Strike",1,"Attack","Common","AnyEnemy",[V("dmg",5),V("power","JudgedPower",1)],[DMG(hits=2),JUDGE(1)],[UP("Damage",2)],f"Deal {{Damage:diff()}} damage twice.\nApply {{JudgedPower:diff()}} {G}Judged{g}.",tips=["JudgedPower"]),
...
# ---------------- Common: neutral
card("Prayer","Prayer",1,"Skill","Common","Self",[V("cards",2)],[DRAW],[UP("Cards",1)],"Draw {Cards:diff()} cards.",bless=True),
...
card("ConsecratedGround","Consecrated Ground",2,"Power","Uncommon","Self",[V("power","ConsecratedGroundPower",1)],[POWERUP,APPLY("ConsecratedGroundPower")],[UP("ConsecratedGroundPower",1)],f"Whenever you play a card that gains {G}Block{g}, all allies gain {{ConsecratedGroundPower:diff()}} {G}Block{g}.",tips=["ConsecratedGroundPower"],stips=["Aura"]),
card("Bastion","Bastion",2,"Skill","Uncommon","Self",[V("blk",8),V("var","Warded",8)],[BLK,WARD],[UP("Block",3),UP("Warded",3)],f"Gain {{Block:diff()}} {G}Block{g} and {{Warded:diff()}} {G}Warded{g}.",tips=["WardedPower"],gb=True),
...
card("ImmovableObject","Immovable Object",2,"Skill","Uncommon","Self",[V("var","Warded",15)],[WARD],[UP("Warded",5)],f"Requires 3 {G}Faith{g} in {G}Torm{g}.\nGain {{Warded:diff()}} {G}Warded{g}.",req=("Torm",3),gb=True,tips=["WardedPower"]),
# ---------------- Uncommon: Ilmater
...
card("AuraOfMercy","Aura of Mercy",2,"Power","Uncommon","Self",[V("power","AuraOfMercyPower",1)],[POWERUP,APPLY("AuraOfMercyPower")],[UP("AuraOfMercyPower",1)],"Whenever an ally is healed, every other ally heals {AuraOfMercyPower:diff()} HP.",mp=True,tips=["AuraOfMercyPower"],stips=["Aura"]),
...
card("Sanctuary","Sanctuary",1,"Skill","Uncommon","AnyPlayer",[V("heal",3),V("blk",5)],[HEAL_TARGET,BLK_TARGET],[UP("Heal",2),UP("Block",3)],f"Heal a player {{Heal:diff()}} HP.\nThey gain {{Block:diff()}} {G}Block{g}.",gb=True,bless=True),
# ---------------- Uncommon: Tyr
card("HolySmite","Holy Smite",1,"Attack","Uncommon","AnyEnemy",[V("dmg",6),V("heal",3)],[DMG(),CAST,"await CreatureCmd.Heal(Owner.Creature, DynamicVars.Heal.BaseValue);"],[UP("Damage",3),UP("Heal",2)],"Deal {Damage:diff()} damage.\nHeal {Heal:diff()} HP.",bless=True),
card("BladeOfJustice","Blade of Justice",1,"Attack","Uncommon","AnyEnemy",[V("dmg",9),V("power","JudgedPower",1)],[DMG(),JUDGE(1),FAITH("Tyr",1)],[UP("Damage",3)],f"Deal {{Damage:diff()}} damage.\nApply {{JudgedPower:diff()}} {G}Judged{g}.\nGain 1 {G}Faith{g} in {G}Tyr{g}.",tips=["JudgedPower"]),
card("Judgment","Judgment",1,"Attack","Uncommon","AnyEnemy",[V("dmg",5),V("power","JudgedPower",2)],[DMG(),JUDGE(2)],[UP("Damage",2),UP("JudgedPower",1)],f"Deal {{Damage:diff()}} damage.\nApply {{JudgedPower:diff()}} {G}Judged{g}.",tips=["JudgedPower"]),
...
card("CrusaderAura","Crusader Aura",2,"Power","Uncommon","Self",[V("power","CrusaderAuraPower",1)],[POWERUP,APPLY("CrusaderAuraPower")],[UP("CrusaderAuraPower",1)],"All allies' Attacks deal {CrusaderAuraPower:diff()} additional damage.",tips=["CrusaderAuraPower"],stips=["Aura"]),
...
card("Litany","Litany",1,"Skill","Uncommon","Self",[],[CAST,"var n = FaithTracks.Highest(Owner).amount / 4;","if (n > 0) await CardPileCmd.Draw(choiceContext, n, Owner, false);"],["EnergyCost.UpgradeBy(-1);"],f"Draw 1 card for every 4 {G}Faith{g} in your highest deity.",bless=True),
...
card("AuraOfDevotion","Aura of Devotion",2,"Power","Rare","Self",[V("power","AuraOfDevotionPower",1)],[POWERUP,APPLY("AuraOfDevotionPower")],[UP("AuraOfDevotionPower",1)],f"All allies take {{AuraOfDevotionPower:diff()}} less damage from attacks for every 5 {G}Faith{g} in {G}Torm{g}.",tips=["AuraOfDevotionPower"],stips=["Aura"]),
...
card("WordOfGlory","Word of Glory",1,"Skill","Rare","AnyPlayer",[V("heal",12)],["if (FaithTracks.Spend(Owner, Deity.Ilmater, 3) > 0) { " + HEAL_TARGET.replace("\n"," ") + " }"],[UP("Heal",4)],f"Requires 3 {G}Faith{g} in {G}Ilmater{g}.\nSpend 3 {G}Faith{g} in {G}Ilmater{g}. Heal a player {{Heal:diff()}} HP.",req=("Ilmater",3),bless=True),
card("AuraOfVitality","Aura of Vitality",2,"Power","Rare","Self",[V("power","AuraOfVitalityPower",3)],[POWERUP,APPLY("AuraOfVitalityPower")],[UP("AuraOfVitalityPower",1)],"At the start of your turn, all allies heal {AuraOfVitalityPower:diff()} HP.",tips=["AuraOfVitalityPower"],stips=["Aura"]),
card("LightOfDawn","Light of Dawn",2,"Skill","Rare","Self",[],[HEAL_ALL("FaithTracks.Effective(Owner, Deity.Ilmater) / 2")],["EnergyCost.UpgradeBy(-1);"],f"Heal all allies HP equal to half your {G}Faith{g} in {G}Ilmater{g}.",bless=True),
...
card("TheBrokenGod","The Broken God",2,"Power","Rare","Self",[V("power","TheBrokenGodPower",1)],[POWERUP,APPLY("TheBrokenGodPower")],["EnergyCost.UpgradeBy(-1);"],f"At the start of your turn, all allies heal HP equal to half your {G}Faith{g} in {G}Ilmater{g}.",tips=["TheBrokenGodPower"],stips=["Aura"]),
# ---------------- Rare: Tyr
card("DivineSmite","Divine Smite",1,"Attack","Rare","AnyEnemy",[V("dmg",8),V("var","Bonus",8),V("power","JudgedPower",2)],["var bonus = FaithTracks.HasAny(Owner, 3) ? DynamicVars[\"Bonus\"].BaseValue : 0m;",DMG_EXPR("DynamicVars.Damage.BaseValue + bonus",fx='"vfx/vfx_attack_blunt"'),JUDGE(2)],[UP("Damage",3),UP("Bonus",3)],f"Deal {{Damage:diff()}} damage.\nIf you have 3 or more {G}Faith{g} in any deity, deal {{Bonus:diff()}} additional damage.\nApply {{JudgedPower:diff()}} {G}Judged{g}.",tips=["JudgedPower"]),
...
def power(name, title, ptype, stack, desc, smart, body, base="PaladinPower", pulse=None):
    return dict(name=name, title=title, ptype=ptype, stack=stack, desc=desc, smart=smart, body=body, base=base, pulse=pulse)
...
power("AuraOfProtection","Aura of Protection","Buff","Counter",
  "Aura. At the start of your turn, all allies gain [blue]2[/blue] [gold]Block[/gold].","Aura. At the start of your turn, all allies gain [blue]{Amount}[/blue] [gold]Block[/gold].",
...
power("ConsecratedGround","Consecrated Ground","Buff","Counter",
  "Aura. Whenever you play a card that gains [gold]Block[/gold], all allies gain [blue]1[/blue] [gold]Block[/gold].","Aura. Whenever you play a card that gains [gold]Block[/gold], all allies gain [blue]{Amount}[/blue] [gold]Block[/gold].",
...
power("AuraOfMercy","Aura of Mercy","Buff","Counter",
  "Aura. Whenever an ally is healed, every other ally heals [blue]1[/blue] HP.","Aura. Whenever an ally is healed, every other ally heals [blue]{Amount}[/blue] HP.",
...
power("Judged","Judged","Debuff","Counter",
  "Takes [blue]1[/blue] additional damage from every Attack, per stack.","Takes [blue]{Amount}[/blue] additional damage from every Attack.",
...
power("Warded","Warded","Buff","Counter",
  "[gold]Block[/gold] up to this amount is not removed at the start of your turn. Each turn, lose [blue]3[/blue] of it.","[gold]Block[/gold] up to [blue]{Amount}[/blue] is not removed at the start of your turn. Each turn, lose [blue]3[/blue] of it.",
...
power("CrusaderAura","Crusader Aura","Buff","Counter",
  "Aura. All allies' Attacks deal [blue]1[/blue] additional damage.","Aura. All allies' Attacks deal [blue]{Amount}[/blue] additional damage.",
  """public override decimal ModifyDamageAdditive(Creature target, decimal amount, ValueProp props, Creature? dealer, CardModel? cardSource)
        => dealer != null && dealer.IsPlayer && cardSource != null && !target.IsPlayer ? amount + Amount : amount;""",
  pulse=f"""Flash();
foreach (var ally in {ALLIES}) await PowerCmd.Apply<VigorPower>(choiceContext, ally, Amount * 2, Owner, null);"""),
...
power("AuraOfDevotion","Aura of Devotion","Buff","Counter",
  "Aura. All allies take [blue]1[/blue] less damage from attacks for every 5 [gold]Faith[/gold] in [gold]Torm[/gold].","Aura. All allies take [blue]{Amount}[/blue] less damage from attacks for every 5 [gold]Faith[/gold] in [gold]Torm[/gold].",
  """public override decimal ModifyDamageAdditive(Creature target, decimal amount, ValueProp props, Creature? dealer, CardModel? cardSource)
    {
  ...
  pulse=f"""Flash();
foreach (var ally in {ALLIES}) await CreatureCmd.GainBlock(ally, 2m, ValueProp.Unpowered, null);"""),
...
power("AuraOfVitality","Aura of Vitality","Buff","Counter",
  "Aura. At the start of your turn, all allies heal [blue]3[/blue] HP.","Aura. At the start of your turn, all allies heal [blue]{Amount}[/blue] HP.",
  """public override async Task AfterPlayerTurnStart(PlayerChoiceContext choiceContext, Player player)
    {
...
power("TheBrokenGod","The Broken God","Buff","Single",
  "Aura. At the start of your turn, all allies heal HP equal to half your [gold]Faith[/gold] in [gold]Ilmater[/gold].","Aura. At the start of your turn, all allies heal HP equal to half your [gold]Faith[/gold] in [gold]Ilmater[/gold].",
  """public override async Task AfterPlayerTurnStart(PlayerChoiceContext choiceContext, Player player)
    {
        if (player.Creature == Owner) await Pulse(choiceContext);
    }""",
  pulse=f"""if (Owner.Player == null) return;
...
def emit_card(c):
    ...
    tipexprs = [f"HoverTipFactory.FromPower<{t}>()" for t in c["tips"]] + [f"Tip(PaladinTips.{t})" for t in c["stips"]]
    if tipexprs: lines.append("    protected override IEnumerable<IHoverTip> ExtraHoverTips => [" + ", ".join(tipexprs) + "];")
    lines.append("")
...
def emit_power(p):
    ...
    lines = ["// <auto-generated> tools/gen_paladin.py -- edit the spec, not this file. </auto-generated>", POWER_USINGS,
             f"/// <summary>{plain}</summary>",
             f'public sealed class {p["name"]}Power : {p["base"]}' + (", IAura" if p.get("pulse") else ""), "{"]
    if own_type:
    ...
    if p.get("pulse"):
        lines.append("")
        lines.append("    /// <summary>The Aura's repeatable effect. Blessed cards call this.</summary>")
        lines.append("    public async Task Pulse(PlayerChoiceContext choiceContext)")
        lines.append("    {")
        for l in p["pulse"].split("\n"): lines.append("        " + l)
        lines.append("    }")
    lines.append("}")
    return "\n".join(lines) + "\n"
